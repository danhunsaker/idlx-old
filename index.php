<?php
	...
	*/
	...
	ob_start();
	session_start();
	if (!isset($_SESSION['user_id']) && !empty($_SERVER['QUERY_STRING'])) {
		error_log ("index.php || User not logged in.  Redirecting to project root.  Doc requested [{$_SERVER['REQUEST_URI']}]");
		...
		if ($iface_nodes->length > 0) {
			...
			foreach ($iface_nodes as $node) {
				...
				if ($iface_contents !== false) {
					error_log("index.php || Importing external Interface [{$script_type}]");
...
//						$node->removeChild($childNode);
		...
		if ($logout_nodes->length > 0) {
			...
			continue;		//	This is only here in case any new checks are added; otherwise, it's redundant.
		}
	}
