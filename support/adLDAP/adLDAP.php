...
class adLDAP {
    ...
    */
    public function authenticate($username, $password, $preventRebind = false) {
		...
		error_log("adLDAP::authenticate || Preparing to auth user [{$username}]");
        ...
        if ($this->useSSO && isset($_SERVER['REMOTE_USER']) && $_SERVER['REMOTE_USER'] == $username && $this->adminUsername === NULL && isset($_SERVER['KRB5CCNAME'])) { 
            putenv("KRB5CCNAME=" . $_SERVER['KRB5CCNAME']);
			error_log("adLDAP::authenticate || Using SSO mode [{$_SERVER['KRB5CCNAME']}]");
            $this->ldapBind = @ldap_sasl_bind($this->ldapConnection, NULL, NULL, "GSSAPI");
            if (!$this->ldapBind) {
                throw new adLDAPException('Rebind to Active Directory failed. AD said: ' . $this->getLastError());
            }
		...
		error_log("adLDAP::authenticate || Using non-SSO mode [{$username} | {$password}]");
        // Bind as the user        
        $ret = true;
